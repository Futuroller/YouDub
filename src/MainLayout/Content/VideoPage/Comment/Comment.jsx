...
function Comment(props) {
    ...
    const handleDelete = async (e, commentId) => {
        try {
            const deletedComment = await apiRequest(`/main/comment/${commentId}`, 'DELETE');
            dispatch(removeHistoryVideo(commentId));
        } catch (error) {
        ...
        <div className={m.container}>
            ...
            <div className={m.comment}>
                <div className={m.topSide}>
                    ...
                    {isUserOwner ? <button className={m.deleteButton} title='Удалить комментарий' onClick={(e) => handleDelete(e, props.id)}></button> : ''}
                </div>
